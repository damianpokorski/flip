...
class AppState {
	...
	isMobile = $state(
		browser && window.matchMedia(MOBILE_BREAKPOINT_QUERY).matches,
	);
	...
	private scheduleEagerFrames() {
		...
		const candidates = this.visibleServices
			...
			.sort(
				(a, b) => (a.ws === activeWs ? 0 : 1) - (b.ws === activeWs ? 0 : 1),
			);
	...
	private enqueueFrame(id: string) {
		if (this.startedFrameIds.has(id) || this.pendingFrameIds.includes(id))
			return;
	...
	openService(id: string) {
		...
		if (!this.recentServiceIds.includes(id)) {
			...
			const outgoing =
				this.recentServiceIds.length === 0 &&
				this.activeServiceId &&
				this.activeServiceId !== id
					? [this.activeServiceId]
					: [];
			this.recentServiceIds = [id, ...outgoing, ...this.recentServiceIds].slice(
				0,
				RECENTS_MAX,
			);
		}
