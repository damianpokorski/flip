...
test.describe("Mobile shell — Rest / Switcher / Settings", () => {
  test("opening a service from the switcher grid activates it, closes the sheet, and surfaces it in the recents bar", async ({
    ...
    try {
      ...
      const card = page.getByTestId("mobile-switcher-card").filter({ hasText: "Mobile Zephyr" });
      ...
      await expect(
        page.locator(`[data-testid="service-iframe"][data-service-id="${body.id}"]`),
      ).toHaveClass(/active/);
      await expect(
        page.locator(`[data-testid="recents-tile"][data-service-id="${body.id}"]`),
      ).toBeVisible();
    } finally {
  ...
  test("tapping the scrim dismisses the switcher without picking a service", async ({ page }) => {
    await page.goto("/");
    ...
    await page.getByLabel("Open switcher").click();
    await expect(page.getByTestId("mobile-switcher")).toBeVisible();
    await page.getByTestId("switcher-scrim").click({ position: { x: 20, y: 20 } });

    // Assert: dismissed, and the previously-active service is untouched.
    await expect(page.getByTestId("mobile-switcher")).toHaveCount(0);
    await expect(
      page.locator(`[data-testid="service-iframe"][data-service-id="${activeId}"]`),
    ).toHaveClass(/active/);
  });
