...
describe("HealthCheckService", () => {
  ...
  test("resolves a local-source service's relative url against this process's own loopback port", async () => {
    // Arrange
    let requestedUrl: string | undefined;
    global.fetch = (async (url: string) => {
      requestedUrl = url;
    ...
    const repo = {
      findAll: async () => [service({ source: "local", url: "/api/sites/demo/" })],
    };
    const health = new HealthCheckService(repo as never);
    ...
    // Assert
    expect(requestedUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+\/api\/sites\/demo\/$/);
  });
