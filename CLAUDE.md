...
- Config/service YAML backwards compatibility relies entirely on zod `.default()` at parse time (`packages/store/src/schemas/*.ts`) — there's no `schemaVersion` field and no migration runner. `YamlFile.readFresh()` backfills a missing field in memory on every read, but never rewrites the file on disk; an old YAML only picks up the new field on disk once something calls `mutate()` (i.e. the next explicit write through the API). Bootstrap's `ensureExists()` only creates missing files from scratch — it never reconciles an existing file against new schema fields. When adding a new optional field to `config.ts`/`service.ts`: give it a `.default(...)`, add a doc comment next to the field, and update the matching `# Fields:` comment block and example value in `defaults.ts`'s `DEFAULT_*_YAML` template — `YamlFile.ensureExists()` self-tests that template against the schema at boot, so a stale/mismatched default fails loudly. Fine for a single-user, human-edited YAML store; revisit only if a real breaking change (field removal/rename/shape change) is ever needed, which zod defaults alone can't handle.
- Mobile has an initial pass, not full parity: a `matchMedia("(max-width: 720px)")` breakpoint (`appState.isMobile` in `app-state.svelte.ts`) swaps the Rest screen's `Sidebar`/`Hud` for a `MobileTopBar` + `RecentsBar` + bottom-sheet `Switcher`, and `/settings` becomes a mobile entry list — but the `Spine` has no mobile equivalent (hidden outright) and settings sub-route forms (the actual create/edit forms) explicitly keep their desktop layout, out of scope for the mobile redesign so far.
...
Biome is the only linter/formatter (`biome.json`) — no ESLint, no Prettier, no Oxlint/Oxfmt. Don't add them. All four scripts invoke Biome directly, not `vp`: `bun run check` (`biome check --write .` — full repo, applies fixes, for local dev), `bun run lint` (`biome ci .` — full repo, non-mutating, this is what CI's "Lint" step runs), `bun run format` (`biome format --write .` — format-only), `bun run staged` (`biome check --write --staged` — Biome's native `--staged` flag, staged files only, applies fixes; this is what the pre-commit hook runs once wired via `bun run hooks:setup`). `vite-plus` (`vp`) is a separate tool used only as the monorepo task runner (`vp run -r/--filter ...` for dev/build/test/check-types) and for the git-hook dispatcher (`vp config`) — it is not involved in linting or formatting. Biome's VCS integration is enabled (`vcs.enabled`/`useIgnoreFile: true` in `biome.json`) so it respects `.gitignore` rather than a hand-maintained ignore list — don't reintroduce a parallel `files.includes` exclusion list for build artifacts. Svelte files intentionally disable a few rules (`useConst`, `useImportType`, unused-vars) since the Svelte compiler handles them differently — don't "fix" this by re-enabling them.
