...
| `bun run docs:screenshots`       | Regenerate the showcase screenshots (README + docs site)       |
...
- **`apps/server`** (Elysia, `:3000` internally, loopback-bound) — the REST API, also reached only through the embedded proxy. Resource controllers for `services` (CRUD + a workspace-scoped reorder + a probe endpoint, embeds live health status) and `workspaces` (CRUD + reorder, cascades a deletion by reassigning member services to the next remaining workspace in one atomic write, refusing the deletion only if it's the last workspace left), plus `config` (read-only — also surfaces the env-derived `proxyDomain`/`proxyPort` for the embedded proxy feature, not just `config.yaml`'s scalar settings). `config`'s `/raw` endpoint returns the live text of the single underlying `config.yaml` file (services and workspaces included), used by the Config settings tab; `sites` (lists folders available under `DATA_DIR/sites/` for the service form's folder picker, and serves their file contents to the services that reference them), and `events` (SSE change/health notifications). OpenAPI docs are generated automatically and available at `/api/openapi`.
- **`apps/docs`** — the usage documentation site (Astro Starlight, published to GitHub Pages). Content is plain markdown in the repo-root `.articles/` (numbered section folders; numeric prefixes are stripped from URLs and `sidebar.order` frontmatter sets ordering). `bun run docs:dev` for a live preview, `bun run docs:build` to build to `apps/docs/dist`. Not shipped in the Docker image.
- **`apps/bootstrap`** — ensures `DATA_DIR` and `config.yaml` exist on startup, migrating a pre-single-file install (separate `services.yaml`/`workspaces.yaml`) forward first if one is found. Runs once before `apps/server` starts (see the Dockerfile `CMD`); `apps/server` also runs the same step itself as a safety net, so local dev never needs this run manually.
- **`packages/store`** — the YAML data layer shared by `apps/server` and `apps/bootstrap`. A single `config.yaml` holds everything: top-level scalar settings (the shared health-check timeout, etc.) plus a `workspaces` array, each workspace holding its own nested `services` array — a service's workspace and its display order are both implicit in where it sits in that structure, not stored fields. `servicesStore`/`workspacesStore`/`configStore` are three facades over one shared `YamlFile` instance, so a cross-resource write (e.g. reassigning a deleted workspace's services to another one) is a single atomic file write, not two independent ones. Reads/writes go through the `yaml` package's `Document` API rather than plain parse/stringify, so comments and formatting on untouched parts of the file survive an edit — including moving a service between workspaces or reordering it, which manipulate the underlying YAML nodes directly rather than rebuilding them from scratch. It also owns `DATA_DIR/sites/` — not a YAML file, just a directory of operator-mounted static site folders, one per `localSlug` referenced by a `source: local` service; `apps/server`'s `sites` controller serves files out of it directly.
